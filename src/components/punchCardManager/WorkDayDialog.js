...
export default function WorkDayDialog({
  ...
  const { callSnackBar, refreshPunchCardData, clientOptions } =
    ...
    await PunchCardService.create(
      {
        ...
        date: averageDateHour(workDayData.date),
      },
      userData.token
    );
  }
    ...
    >
      ...
      <DialogContent>
        ...
        <DialogContentText>
          {`Status atual:   `}
          <Button variant="text">
            {(workDayData.isPresence && mode === "preview") ||
            (!workDayData.isPresence && mode === "edit")
              ? "PRESENÇA"
              : "FALTA"}
          </Button>
        </DialogContentText>
        ...
        <FormControl fullWidth>
          ...
          <Select
            ...
            value={
              mode === "preview"
                ? clients.options.find(
                    (client) => client.id === workDayData.clients?.id
                  )?.id ?? ""
                : clients.selectedId
            }
            label="Obra"
            ...
            onChange={(e) =>
              setClients((prev) => ({ ...prev, selectedId: e.target.value }))
            }
          >
            {clients.options.map((client) => (
      ...
      <DialogActions>
        {isLoading ? (
          <WorkDayDialogLoadingContainer>
            <LogoLoadingSpinner width="60px" height="60px" />
          </WorkDayDialogLoadingContainer>
        ) : (
          <>
