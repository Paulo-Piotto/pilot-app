...
import { EmployeesService, PunchCardService } from "../../services/api.services"
import { useState, useContext } from "react"
import PunchCardContext from "../context/PunchCardContext"
import Loader from "../generics/logoLoadingSpinner";
import AuthContext from "../context/AuthContext"

export default function SearchCard({contrast, plHolder, iconName, action, width}) {
    const { setPunchCardData, callSnackBar, updateSearchFilters } = useContext(PunchCardContext);
    ...
    async function searchEmployee() {
        setLoading(true)
        try {
            const foundEmployees = await EmployeesService.searchEmployeeByName(searchString)
            setPunchCardData(prev => ({
                ...prev,
                byEmployees: foundEmployees.data
            }))
        } catch(error) {
            if(error.response.status === 404) callSnackBar({ message: "Nenhum funcionário encontrado", type: "error" })
            console.error(error)
        }
        setLoading(false)
    }
    ...
    async function getEmployeesWithoutPunchCardRecord() {
        setLoading(true)
        const emptyPunchs = await PunchCardService.getEmptyPunchCards(userData.token)
        setPunchCardData(prev => ({
            ...prev,
